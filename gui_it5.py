...
class EyeAnalyzerApp:
    def __init__(self, master):
        ...
        self.face_cascade = cv2.CascadeClassifier('haarcascade_eye.xml')
        self.wrinkle_model = self.load_model("drowsy.json", "drowsy_detector.h5")

        self.WRINKLE_CLASSES = ["Eye open", "Eye close"]
    ...
    def show_detect_buttons(self, file_path):
        detect_wrinkle_button = Button(self.master, text="Eye Detection", command=lambda: self.detect_wrinkle(file_path), padx=10, pady=5)
    ...
    def detect_wrinkle(self, file_path):
        try:
            ...
            print("Predicted Eye Status: " + pred)
            self.label_result.configure(foreground="#011638", text=f"Eye Status: {pred}")
        except Exception as e:
            self.label_result.configure(foreground="#011638", text="Unable to detect")

    def load_model(self, drowsy, drowsy_detector):
        try:
            with open(drowsy, "r") as file:
                loaded_model_json = file.read()
                model = model_from_json(loaded_model_json)

            model.load_weights(drowsy)
